...
const double LIGTH_VELOCITY      = 299792458.0;
...
/*function[a, b, alpha] = envl2ellip(s11, s12, s22)

alpha = -atan(2 * s12 / (s22 - s11)) / 2;
a = std::abs(sqrt(s11 - 2 * std::sin(-alpha) ^ 2 * s12 / std::sin(-2 * alpha)));
b = std::abs(sqrt(s11 + 2 * std::cos(-alpha) ^ 2 * s12 / std::sin(-2 * alpha)));
                            ...
                            int flag, int Distrtype, std::vector<int> props, int numberOfTraces)
{
    ...
    case 0:
        ...
        en = LIGTH_VELOCITY * LIGTH_VELOCITY * data->mass;
        ...
        for (int i = 0; i < data->data[0].size(); i++)
        {
            ...
            {
                float V   = tmp[i][j] * LIGTH_VELOCITY;
                tmp[i][j] = data->mass * V * V / (std::abs(Dconst::ELECTRON_CHARGE) * 2);
            }
        }
    ...
    case 1:
        ...
        for (int k = 0; k < data->data[0][0].size(); k++)
        {
            ...
            for (int i = 0; i < data->TimeArray.size(); i++)
            {
                ...
                tmp[i][k] = 2 * Dconst::PI * tmp[i][k] / (beta[k] * data->lambda);
            }
        }
                          ...
                          int flag, int Distrtype, std::vector<int> props, int numberOfTraces)
{
    ...
    case 10:
        ...
        for (int i = 0; i < tmp.size(); i++)
            tmp[i] = tmp[i] * LIGTH_VELOCITY;

        float omega = 2 * Dconst::PI * 108e6;

        for (int i = 0; i < tmp.size(); i++)
        {
            tmpT[i] = tmpT[i] * 1e-9 * omega;
            while (tmpT[i] > Dconst::PI)
                tmpT[i] = tmpT[i] - 2 * Dconst::PI;
        }
        ...
        for (int i = 0; i < data->dataAdd[14].size(); i++)
        {
            ...
            while (tmpT1[i] > Dconst::PI / 2)
                tmpT1[i] = tmpT1[i] - Dconst::PI;
        }
...
void ShowAllCloud(vtkComponent* input, ModelInterface* currentModel, int precisionType, int problemType)
{
    ...
    for (int k = 0; k < currentModel->GetNumberParticlesFlows(); k++)
    {
        ...
        if (k == 1)
            input->addVisualizationDataCloud(pointCloud, 0.5, colors::blackcolor);
        else
            input->addVisualizationDataCloud(pointCloud, 0.5);
    }
    // input->addVisualizationDataBoundary(currentModel->GetMeshBoundaryVTKGrid(), colors::redcolor);
                           ...
                           int emFlag, int nCharts)
{
    ...
    if (emFlag <= 4)
    {
        ...
        input->addVisualizationDataPlot(data[0], data[1], colors::blackcolor, 5, xName, yName, 1, 1, 0);
    }
    else
    {
        volatile int N = 1;
        ...
        input->addVisualizationDataPlot(XChart, YChart, colors::blackcolor, 5, xName, yName, 2, 1, 0);
    }
};
...
void ShowEmitterField(vtkComponent* input, ModelInterface* currentModel, int flowNumber)
{
    ...
    std::vector<std::vector<float>> tmp   = currentModel->GetEmitterField(flowNumber);
    std::string                     xName = "Emitter Length";
...
void Show2daxsTrace(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data,
                    int flowNumber, int numberOfTraces, int flag, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
    std::string yName, xName;
    ...
    case 2:
        yName = "Phi, rad";
        xName = "T, ns";
        input->addVisualizationDataPlotsAutoY(data->TimeArray, data->data[4], colors::blackcolor, 1, xName, yName,
                                              numberOfTraces, Distrtype, props, ymax, ymin);
        break;
    case 3:
        yName = "Charge";
        xName = "T, ns";
        input->addVisualizationDataPlotsAutoY(data->TimeArray, data->data[6], colors::blackcolor, 1, xName, yName,
                                              numberOfTraces, Distrtype, props, ymax, ymin);
        break;
    case 4:
        yName = "R, m";
...
void ShowEnergy3d(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data, int flowNumber,
                  int numberOfTraces, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
    ...
    float en = LIGTH_VELOCITY * LIGTH_VELOCITY * prop[2];
...
void ShowChargeT(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data, int flowNumber,
                 int numberOfTraces, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
    ...
    std::string yName = "charge";
    std::string xName = "T, ns";
    input->addVisualizationDataPlots(data->TimeArray, data->data[6], colors::blackcolor, 1, xName, yName,
                                     numberOfTraces, Distrtype, props, ymax, ymin);
};
            ...
            int numberOfTraces, int Distrtype, std::vector<int> props)
{
    ...
    for (int i = 0; i < indexes.size(); i++)
    {
        ...
        {
            Xtmp.push_back(data->data[0][indexes[i]][j] * std::sin(data->data[4][indexes[i]][j]));
            Ytmp.push_back(data->data[0][indexes[i]][j] * std::cos(data->data[4][indexes[i]][j]));
        }
        if (Xtmp.size() > 1)
...
void ShowEnergy(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data, int flowNumber,
                int numberOfTraces, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
...
void ShowEnergyEZ(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data, int flowNumber,
                  int numberOfTraces, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
    std::string         yName = "Energy, eV";
    std::string         xName = "Z, m";
    std::vector<double> prop  = currentModel->GetFlowProperties(flowNumber);
    float               ymax, ymin;
    ...
    input->addVisualizationDataPlotsAutoY(data->data[1], Ydata, colors::blackcolor, 1, xName, yName, numberOfTraces,
                                          Distrtype, props, ymax, ymin);
}
...
void ShowEnergy2d(vtkComponent* input, ModelInterface* currentModel, std::shared_ptr<DynamicsData> data, int flowNumber,
                  int numberOfTraces, int Distrtype, std::vector<int> props)
{
    input->CheckType(1);
    input->clear();
    ...
    for (int i = 0; i < data->data[0].size(); i++)
    {
        ...
        for (int j = 0; j < data->data[0][i].size(); j++)
        {
            gamma = sqrt(1 + data->data[2][i][j] * data->data[2][i][j] + data->data[3][i][j] * data->data[3][i][j]);
            Xtmp.push_back(-(gamma - 1) * en / ELECTRON_CHARGE);
        }
        Ydata.push_back(Xtmp);
    }
...
void ShowValueAlongConductor(vtkComponent* input, ModelInterface* currentModel, int conductor, int flag)
{
    ...
    if (flag == 0)
        yName = "Av. irr. power density, W/(m*m)";

    if (flag == 1)
        yName = "Av. irr. current density, A/(m*m)";
