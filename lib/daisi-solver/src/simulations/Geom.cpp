...
namespace DGeo {
    ...
    bool Point<PointType>::isEqual(Point<PointType> point1, PointType epsilon) const {
        return std::abs(point1.x - x) < epsilon && std::abs(point1.y - y) < epsilon && std::abs(point1.z - z) < epsilon;
    }
    ...
    template<class PointType>
    inline Point<PointType>::Point() {
        isOut = false;
    }

    template<class PointType>
    inline Point<PointType>::Point(Point<PointType> Old, PointType hx, PointType hy, PointType hz) {
        *this = Old;
        x += hx;
        y += hy;
        z += hz;
