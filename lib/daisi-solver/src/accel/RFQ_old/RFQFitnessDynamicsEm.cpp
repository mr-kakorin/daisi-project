...
double LinacTools::RFQFitnessDynamicsEm::FitnessFunction(const std::vector<double>& population)
{
/*     std::vector<double> MinumalRadiiRegular;
    ...
    LinacTools::GenerateRFQForFlow(succes, RFQParameters, flows[0], Modulations, SyncPhases, CellsLengths, MinumalRadii,
                                   MinumalRadiiRegular, MatcherRadii, OutRadii, AccEff, AvEnergies);
    for (int i = 0; i < flows.size(); i++)
    {
        ...
        dynamicsSimulator.CalculateRFQAcceptance(0, flows[i], RFQParameters, CellsLengths, Modulations, MinumalRadii,
                                                 MatcherRadii, AccEff, outputData);
        outputData.clear();
        dynamicsSimulator.SimulateRFQFlowDynamics(0, nCells, flows[i], RFQParameters, CellsLengths, MinumalRadii,
                                                  AccEff, outputData, SyncPhases[0]);

        if (outputData.back()->dataAdd[9].size() < flows[i].nParticlesTransv)
        {
            result = 1;
            continue;
        }
        ...
        for (int np = 0; np < outputData.back()->dataAdd[9].size(); np++)
        {
            Sav = Sav + outputData.back()->dataAdd[9][np];
        }
        Sav = Sav / outputData.back()->dataAdd[9].size();

        for (int np = 0; np < outputData.back()->dataAdd[9].size(); np++)
        {
            result = result + (Sav - outputData.back()->dataAdd[9][np]) * (Sav - outputData.back()->dataAdd[9][np]);
        }

        Sav = 0;

        for (int np = 0; np < outputData.back()->dataAdd[12].size(); np++)
        {
            Sav = Sav + outputData.back()->dataAdd[12][np];
        }
        Sav = Sav / outputData.back()->dataAdd[12].size();

        for (int np = 0; np < outputData.back()->dataAdd[12].size(); np++)
        {
            result = result + (Sav - outputData.back()->dataAdd[12][np]) * (Sav - outputData.back()->dataAdd[12][np]);
        }
    }
    return result; */
};
void LinacTools::RFQFitnessDynamicsEm::CheckPopulation(std::vector<double>& population)
{
...
void LinacTools::RFQFitnessDynamicsEm::GeneratePopulation(std::vector<double>& population)
{
  /*   population.clear();
    ...
    std::vector<double> sigmas = {0,    0, 0.05, 0, 0.5, 0,    0, 0.05, 0, 0.5,   0, 0,
                                  0.05, 0, 0.5,  0, 0,   0.05, 0, 0,    1, 0.005, 0, 0};
