...
double LinacTools::RFQFitnessDynamics::FitnessFunction(const std::vector<double>& population){
    /*     std::vector<double> MinumalRadiiRegular;
       ...
       AvEnergies);
        for (int i = 0; i < flows.size(); i++)
        {
            ...
            double Transmission;
            dynamicsSimulator.SimulateRFQLongDynamics(0, 0, nCells, flows[i], RFQParameters,
       CellsLengths, MinumalRadii,
                                                      AccEff, outputData, Transmission,
       SyncPhases[0]);
            result = result + (1 - Transmission);
        }
        return result; */
};
void LinacTools::RFQFitnessDynamics::CheckPopulation(std::vector<double>& population){
...
void LinacTools::RFQFitnessDynamics::GeneratePopulation(std::vector<double>& population){
    /*    population.clear();
