...
{
  public:
    JSONWriter(const T& object) : BaseWriter<T, boost::property_tree::ptree, JSONWriter<T>>(object)
    {
    }
    ...
    template <typename Tvector>
    void write_objects_vector(const std::vector<Tvector>& element, const std::string& name) const
    {
        boost::property_tree::ptree children;
        for (const auto& val : element)
        {
